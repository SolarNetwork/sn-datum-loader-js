// https://github.com/SolarNetwork/sn-datum-loader-js Version 0.6.1-dev.0. Copyright 2017 Matt Magoffin.
(function (global, factory) {
	...
	(factory((global.sn = global.sn || {}),global.d3,global.sn,global.d3));
...
var DatumLoader = function (_JsonClientSupport) {
  ...
  */
    ...
    */
		...
		/** @type {DatumFilter} */
		_this.filter = filter || new solarnetworkApiCore.DatumFilter({ nodeIds: _this.urlHelper.nodeIds });
   ...
   */
		...
		value: function loadData(page) {
			...
			jsonClient(url).on('beforesend', function (request) {
				...
				request.setRequestHeader(solarnetworkApiCore.HttpHeaders.X_SN_DATE, authBuilder.requestDateHeaderValue);
				request.setRequestHeader(solarnetworkApiCore.HttpHeaders.AUTHORIZATION, authBuilder.buildWithSavedKey());
			}).on('load', function (json$$1) {
				var dataArray = datumExtractor(json$$1);
				if (dataArray === undefined) {
					solarnetworkApiCore.Logger.debug('No data available for %s', url);
				...
				solarnetworkApiCore.Logger.error('Error requesting data for %s: %s', url, error);
