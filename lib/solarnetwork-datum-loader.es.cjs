// https://github.com/SolarNetwork/sn-datum-loader-js Version 2.0.4-dev.0. Copyright 2025 SolarNetwork Foundation.
'use strict';

var index_js = require('solarnetwork-api-core/lib/net/index.js');
var index_js$1 = require('solarnetwork-api-core/lib/util/index.js');
var index_js$2 = require('solarnetwork-api-core/lib/domain/index.js');
    ...
    requestor(url, signUrl) {
        ...
        return (cb) => {
            ...
            if (auth && auth.signingKeyValid) {
                headers[index_js.HttpHeaders.AUTHORIZATION] = auth
                    .reset()
                    .snDate(true)
                    .url(signUrl || url, true)
                    .buildWithSavedKey();
                headers[index_js.HttpHeaders.X_SN_DATE] = auth.requestDateHeaderValue;
            }
            const errorHandler = (error) => {
                index_js$1.Logger.error("Error requesting data for %s: %s", url, error);
...
class DatumLoader extends JsonClientSupport {
    ...
    load(callback) {
        ...
        this.#loadData(new index_js$2.Pagination(this.#pageSize, 0));
        return this;
    }
    /**
     ...
     */
    #loadData(page, q) {
        const queryFilter = new index_js$2.DatumFilter(this.filter);
        queryFilter.withoutTotalResultsCount =
            (this.#includeTotalResultsCount || q) && page.offset === 0
                ? false
                : true;
        let url = this.#readingsMode
            ? this.api.datumReadingUrl(index_js$2.DatumReadingTypes.Difference, queryFilter, undefined, undefined, page)
            : this.api.listDatumUrl(queryFilter, undefined, page);
        if (this.#urlParameters) {
            const queryParams = index_js.Urls.urlQueryEncode(this.#urlParameters);
            if (queryParams) {
                url += "&" + queryParams;
            }
        }
        ...
        const handler = (error, data) => {
            ...
            if (dataArray === undefined) {
                index_js$1.Logger.debug("No data available for %s", reqUrl);
                if (!q) {
...
class DatumRangeFinder extends JsonClientSupport {
    ...
    load(callback) {
        ...
        q.awaitAll((error, results) => {
            if (error) {
                index_js$1.Logger.error("Error requesting available data range: %s", error);
                callback(error);
                return;
            }
    ...
    #extractReportableInterval(results) {
        ...
        for (i = 0; i < results.length; i += 1) {
            ...
            if (repInterval?.endDate === undefined) {
                index_js$1.Logger.debug("No data available for %s sources %s", this.#filters[i].nodeId, this.#filters[i].sourceIds !== undefined
...
class DatumSourceFinder extends JsonClientSupport {
    ...
    load(callback) {
        ...
        q.awaitAll((error, results) => {
            if (error || !results) {
                index_js$1.Logger.error("Error requesting available sources: %s", error);
                callback(error);
                return;
            }
            const result = {};
