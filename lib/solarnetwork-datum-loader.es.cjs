// https://github.com/SolarNetwork/sn-datum-loader-js Version 3.0.1-dev.0. Copyright 2025 SolarNetwork Foundation.
'use strict';

var solarnetworkApiCore = require('solarnetwork-api-core');
    ...
    requestor(url, signUrl) {
        ...
        return (cb) => {
            ...
            if (auth && auth.signingKeyValid) {
                headers[solarnetworkApiCore.Net.HttpHeaders.AUTHORIZATION] = auth
                    .reset()
                    .snDate(true)
                    .url(signUrl || url, true)
                    .buildWithSavedKey();
                headers[solarnetworkApiCore.Net.HttpHeaders.X_SN_DATE] =
                    auth.requestDateHeaderValue;
            }
            const errorHandler = (error) => {
                solarnetworkApiCore.Util.Logger.error("Error requesting data for %s: %s", url, error);
...
class DatumLoader extends JsonClientSupport {
    ...
    load(callback) {
        ...
        this.#loadData(new solarnetworkApiCore.Domain.Pagination(this.#pageSize, 0));
        return this;
    }
    /**
     ...
     */
    #loadData(page, q) {
        const queryFilter = new solarnetworkApiCore.Domain.DatumFilter(this.filter);
        queryFilter.withoutTotalResultsCount =
            (this.#includeTotalResultsCount || q) && page.offset === 0
                ? false
                : true;
        let url = this.#readingsMode
            ? this.api.datumReadingUrl(solarnetworkApiCore.Domain.DatumReadingTypes.Difference, queryFilter, undefined, undefined, page)
            : this.api.listDatumUrl(queryFilter, undefined, page);
        if (this.#urlParameters) {
            const queryParams = solarnetworkApiCore.Net.Urls.urlQueryEncode(this.#urlParameters);
            if (queryParams) {
                url += "&" + queryParams;
            }
        }
        ...
        const handler = (error, data) => {
            ...
            if (dataArray === undefined) {
                solarnetworkApiCore.Util.Logger.debug("No data available for %s", reqUrl);
                if (!q) {
...
class DatumRangeFinder extends JsonClientSupport {
    ...
    load(callback) {
        ...
        q.awaitAll((error, results) => {
            if (error) {
                solarnetworkApiCore.Util.Logger.error("Error requesting available data range: %s", error);
                callback(error);
                return;
            }
    ...
    #extractReportableInterval(results) {
        ...
        for (i = 0; i < results.length; i += 1) {
            ...
            if (repInterval?.endDate === undefined) {
                solarnetworkApiCore.Util.Logger.debug("No data available for %s sources %s", this.#filters[i].nodeId, this.#filters[i].sourceIds !== undefined
...
class DatumSourceFinder extends JsonClientSupport {
    ...
    load(callback) {
        ...
        q.awaitAll((error, results) => {
            if (error || !results) {
                solarnetworkApiCore.Util.Logger.error("Error requesting available sources: %s", error);
                callback(error);
                return;
            }
            const result = {};
