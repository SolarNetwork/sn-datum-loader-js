...
var net = require('solarnetwork-api-core/lib/net');
var util = require('solarnetwork-api-core/lib/util');
var domain = require('solarnetwork-api-core/lib/domain');
        ...
        return (cb) => {
            ...
            if (auth && auth.signingKeyValid) {
                headers[net.HttpHeaders.AUTHORIZATION] = auth
                    ...
                    .buildWithSavedKey();
                headers[net.HttpHeaders.X_SN_DATE] = auth.requestDateHeaderValue;
            }
            const errorHandler = (error) => {
                util.Logger.error("Error requesting data for %s: %s", url, error);
...
class DatumLoader extends JsonClientSupport {
    ...
    #loadData(page, q) {
        ...
        let url = this.#readingsMode
            ? this.api.datumReadingUrl(domain.DatumReadingTypes.Difference, queryFilter, undefined, undefined, page)
...
class DatumRangeFinder extends JsonClientSupport {
    ...
    load(callback) {
        ...
        q.awaitAll((error, results) => {
            if (error) {
                util.Logger.error("Error requesting available data range: %s", error);
                callback(error);
                return;
            }
    ...
    #extractReportableInterval(results) {
        ...
        for (i = 0; i < results.length; i += 1) {
            ...
            if (repInterval?.endDate === undefined) {
                util.Logger.debug("No data available for %s sources %s", this.#filters[i].nodeId, this.#filters[i].sourceIds !== undefined
...
class DatumSourceFinder extends JsonClientSupport {
    ...
    load(callback) {
        ...
        q.awaitAll((error, results) => {
            if (error || !results) {
                util.Logger.error("Error requesting available sources: %s", error);
                callback(error);
                return;
            }
            const result = {};
