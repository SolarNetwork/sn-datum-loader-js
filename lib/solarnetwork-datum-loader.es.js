// https://github.com/SolarNetwork/sn-datum-loader-js Version 0.10.1. Copyright 2019 Matt Magoffin.
import { queue } from 'd3-queue';
import { DatumFilter, DatumReadingTypes, HttpHeaders, Logger, NodeDatumUrlHelper, Pagination, urlQuery } from 'solarnetwork-api-core';
...
var JsonClientSupport = function () {
	/**
  * Constructor.
  *
  ...
  */
	function JsonClientSupport(authBuilder) {
		classCallCheck(this, JsonClientSupport);
...
var DatumRangeFinder = function (_JsonClientSupport) {
	inherits(DatumRangeFinder, _JsonClientSupport);
  ...
  */
	function DatumRangeFinder(urlHelpers, authBuilder) {
		...
		var _this = possibleConstructorReturn(this, (DatumRangeFinder.__proto__ || Object.getPrototypeOf(DatumRangeFinder)).call(this, authBuilder));
   ...
   * @private
   */
		_this._helpers = Array.isArray(urlHelpers) ? urlHelpers : urlHelpers ? [urlHelpers] : [new NodeDatumUrlHelper()];
		return _this;
	}
...
var DatumSourceFinder = function (_JsonClientSupport) {
	inherits(DatumSourceFinder, _JsonClientSupport);
  ...
  */
	function DatumSourceFinder(urlHelpers, authBuilder) {
		classCallCheck(this, DatumSourceFinder);

		var _this = possibleConstructorReturn(this, (DatumSourceFinder.__proto__ || Object.getPrototypeOf(DatumSourceFinder)).call(this, authBuilder));
		...
		value: function load(callback) {
			...
			try {
				for (var _iterator = this._helpers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
					...
					var filter = new DatumFilter(this.datumFilter);
					...
					if (filter.metadataFilter || filter.nodeIds.length === 1) {
						...
						try {
							for (var _iterator4 = filter.nodeIds[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
								...
								var oneFilter = new DatumFilter(filter);
...
var DatumLoader = function (_JsonClientSupport) {
	inherits(DatumLoader, _JsonClientSupport);
  ...
  */
	function DatumLoader(urlHelper, filter, authBuilder) {
		...
		var _this = possibleConstructorReturn(this, (DatumLoader.__proto__ || Object.getPrototypeOf(DatumLoader)).call(this, authBuilder));
    ...
    */
		...
		/** @type {NodeDatumUrlHelper} */
		_this.urlHelper = urlHelper || new NodeDatumUrlHelper();
		if (!authBuilder) {
			urlHelper.publicQuery = true;
		}

		/** @type {DatumFilter} */
		_this.filter = filter || new DatumFilter({
  ...
  */
   ...
   */
		...
		value: function loadData(page) {
			...
			var queryFilter = new DatumFilter(this.filter);
			queryFilter.withoutTotalResultsCount = (this._includeTotalResultsCount || q) && pagination.offset === 0 ? false : true;

			var url = this._readingsMode ? this.urlHelper.datumReadingUrl(queryFilter, DatumReadingTypes.Difference, undefined, undefined, pagination) : this.urlHelper.listDatumUrl(queryFilter, undefined, pagination);
			...
			var req = jsonClient(reqUrl).on("beforesend", function (request) {
				...
				var nextOffset = offsetExtractor(json$$1, pagination);
				var totalResults = json$$1.data ? json$$1.data.totalResults : null;

				if (_this2._results === undefined || incMode) {
					_this2._results = dataArray;
				...
				Logger.error("Error requesting data for %s: %s", reqUrl, error);
				_this2.handleResults(new Error("Error requesting data for " + reqUrl + ": " + error));
			});
 ...
 * @private
 */
function offsetExtractor(json$$1, page) {
	var data = json$$1.data;
	if (page && page.max) {
...
export { DatumRangeFinder, DatumSourceFinder, DatumLoader, MultiLoader };
