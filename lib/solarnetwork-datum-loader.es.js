// https://github.com/SolarNetwork/sn-datum-loader-js Version 0.7.1. Copyright 2018 Matt Magoffin.
  ...
  */
   ...
   */
		...
		value: function loadData(page) {
			...
			queryFilter.withoutTotalResultsCount = this._includeTotalResultsCount && pagination.offset === 0 ? false : true;

			var url = this.urlHelper.listDatumUrl(queryFilter, undefined, pagination);
			...
			var jsonClient = this.client();
			jsonClient(url).on('beforesend', function (request) {
				if (auth && auth.signingKeyValid) {
					auth.reset().snDate(true).url(url);
					request.setRequestHeader(HttpHeaders.X_SN_DATE, auth.requestDateHeaderValue);
					request.setRequestHeader(HttpHeaders.AUTHORIZATION, auth.buildWithSavedKey());
