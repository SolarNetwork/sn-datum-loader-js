...
import { Util } from "solarnetwork-api-core";
 ...
 * filter.nodeId = 123;
 * const sources = await new DatumSourceFinder(new Net.SolarQueryApi(), filter).fetch();
 *
 * @example
 * // find all sources matching a wildcard pattern within the past day
 * const filter2 = new Domain.DatumFilter();
 * filter2.startDate = new Date(Date.now() - 24 * 60 * 60 * 1000);
 * filter2.sourceId = '/power/**';
 * const sources2 = await new DatumSourceFinder(new Net.SolarQueryApi(), filter2).fetch();
 *
 * @example
 * // find all sources across multiple SolarNodes
 * const filter3 = new Domain.DatumFilter();
 * filter3.nodeId = 234;
 * const sources3 = await new DatumSourceFinder(new Net.SolarQueryApi(), [urlHelper1, urlHelper3]).fetch();
 */
